...
{
    /// <summary>
    /// Предоставляет методы конфигурации модели движимого имущества.
    /// </summary>
    public static class MovableEstateModel
    {
        private const string MovableEstate_AdditionalFeatures = nameof(MovableEstate_AdditionalFeatures);

        /// <summary>
        /// Создает конфигурацию модели движимого имущества по умолчанию.
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        public static void CreateModelConfig(this IInitializerContext context)
        {
            context.CreateVmConfig<AdditionalFeatures>(MovableEstate_AdditionalFeatures);

            context.CreateVmConfig<MovableEstate>()
                   .Service<IMovableEstateService>()
                   .Title("Движимое имущество")
                   .DetailView_Default()
                   .ListView_Default()
                   .LookupProperty(x => x.Text(c => c.Name));

            context.CreateVmConfig<MovableEstate>("MovableEstateMenuList")
                     .Service<IMovableEstateService>()
                     .Title("Движимое имущество")
                     .DetailView_Default()
                     .ListView_MovableEstateMenuList()
                     .LookupProperty(x => x.Text(c => c.Name));

        }

        /// <summary>
        /// Конфигурация карточки движимого имущества по умолчанию.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<MovableEstate> DetailView_Default(this ViewModelConfigBuilder<MovableEstate> conf)
        {
               ...
               .Editors(editors => editors
                //Доп. характеристики
                .AddPartialEditor(MovableEstate_AdditionalFeatures, nameof(IHasAdditionalFeature.AdditionalFeaturesID), nameof(IBaseObject.ID),
                ...
                .Add(ed => ed.Name, ac => ac.Title("Наименование БУ").TabName(EstateTabs.GeneralInfo).IsRequired(true).Visible(true).Order(20))
                 // .Add(ed => ed.EstateType, ac => ac.Title("Класс КС").TabName(EstateTabs.GeneralInfo).Visible(true).Order(30))
                  ...
                  .AddOneToManyAssociation<AccountingObject>("MovableEstate_AccountingObjects",
                        editor => editor
                  ...
                  .AddManyToMany("MovableEstate_FileCards"
        ...
        /// <summary>
        /// Конфигурация реестра движимого имущества по умолчанию.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<MovableEstate> ListView_Default(this ViewModelConfigBuilder<MovableEstate> conf)
        {
            return
            conf.ListView(x => x
                .Title("Движимое имущество")
                ...
                .ColumnsFrom<AdditionalFeatures>(MovableEstate_AdditionalFeatures, nameof(IHasAdditionalFeature.AdditionalFeaturesID), nameof(IBaseObject.ID))
                );
        }

        /// <summary>
        /// Конфигурация реестра движимого имущества для мнемоники реестра, отображаемого при открытии пункта навигационного меню.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<MovableEstate> ListView_MovableEstateMenuList(this ViewModelConfigBuilder<MovableEstate> conf)
        {
            return
