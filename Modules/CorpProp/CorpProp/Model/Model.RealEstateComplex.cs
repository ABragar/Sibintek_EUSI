...
{
    /// <summary>
    /// Предоставляет методы конфигурации модели ЕНК.
    /// </summary>
    public static class RealEstateComplexModel
    {
        private const string RealEstateComplex_AdditionalFeatures = nameof(RealEstateComplex_AdditionalFeatures);
        ...
        {
            context.CreateVmConfig<AdditionalFeatures>(RealEstateComplex_AdditionalFeatures);

            context.CreateVmConfig<RealEstateComplex>()
                   .Service<IRealEstateComplexService>()
                   .Title("Единый недвижимый комплекс")
                   .DetailView_Default()
                   .ListView_Default()
                   .LookupProperty(x => x.Text(c => c.Name));

            context.CreateVmConfig<RealEstateComplex>("RealEstateComplexMenuList")
                     .Service<IRealEstateComplexService>()
                     .Title("Единый недвижимый комплекс")
                     .DetailView_Default()
                     .ListView_RealEstateComplexMenuList()
                     .LookupProperty(x => x.Text(c => c.Name));

        }

        /// <summary>
        /// Конфигурация карточки кадастрового объекта по умолчанию.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<RealEstateComplex> DetailView_Default(this ViewModelConfigBuilder<RealEstateComplex> conf)
        {
           ...
           .Editors(editors => editors
                //Доп. характеристики
                .AddPartialEditor(RealEstateComplex_AdditionalFeatures, nameof(IHasAdditionalFeature.AdditionalFeaturesID), nameof(IBaseObject.ID),
              ...
              .Add(ed => ed.Address, ac => ac.Title("Адрес (местоположение)").TabName(EstateTabs.GeneralInfo).Visible(false).Order(150))
               ...
               .Add(ed => ed.Name, ac => ac.Title("Наименование БУ").TabName(EstateTabs.Characteristics3).Visible(true).Order(10))
               .Add(ed => ed.NameByRight, ac => ac.Title("Наименование по данным ЕГРН").TabName(EstateTabs.Characteristics3).Visible(true).Order(11))
               .Add(ed => ed.RealEstatePurpose, ac => ac.Title("Назначение здания/сооружения").TabName(EstateTabs.Characteristics3).Visible(true).Order(12))
               .Add(ed => ed.RealEstateKinds, ac => ac.Title("Вид(ы) разрешенного использования").TabName(EstateTabs.Characteristics3).Visible(true).Order(13))
               ...
               //сведения о частях
               .AddOneToManyAssociation<CadastralPart>("RealEstateComplex_CadastralParts",
                     ...
                     .Title("Сведения о частях")
                         ...
                         .Filter((uofw, q, id, oid) => q.Where(w => w.CadastralID == id)).Visible(true).Order(16))
                .Add(ed => ed.RealEstateKind, ac => ac.Title("Вид объекта недвижимости").TabName(EstateTabs.Characteristics3).Visible(true).Order(15))

               //координаты
               .Add(ed => ed.Location, ac => ac.TabName("Местоположение").Visible(true).Order(12))
               .AddOneToManyAssociation<Coordinate>("RealEstateComplex_LandLocations",
                     y => y.TabName(EstateTabs.Coordinates4)
                     .Title("Местоположения границ ЗУ, в пределах которых расположен ЕНК")                     
                     .IsLabelVisible(true)
                         .Create((uofw, entity, id) =>
                         {
                             entity.Cadastral = uofw.GetRepository<Cadastral>().Find(id);
                         ...
                         .Filter((uofw, q, id, oid) => q.Where(w => w.CadastralID == id && w.EstateTyepLocation == EstateTyepLocation.Land)).Visible(true).Order(1))

                .AddOneToManyAssociation<Coordinate>("RealEstateComplex_BuildLocations",
                     y => y.TabName(EstateTabs.Coordinates4)
                     .Title("Местоположения зданий, входящих в ЕНК")
                     .IsLabelVisible(true)
                         .Create((uofw, entity, id) =>
                         {
                             entity.Cadastral = uofw.GetRepository<Cadastral>().Find(id);
                         ...
                         .Filter((uofw, q, id, oid) => q.Where(w => w.CadastralID == id && w.EstateTyepLocation == EstateTyepLocation.Build)).Visible(true).Order(2))

                .AddOneToManyAssociation<Coordinate>("RealEstateComplex_ConstructionLocations",
                     y => y.TabName(EstateTabs.Coordinates4)
                     .Title("Местоположения сооружений, входящих в ЕНК")
                     .IsLabelVisible(true)
                         .Create((uofw, entity, id) =>
                         {
                             entity.Cadastral = uofw.GetRepository<Cadastral>().Find(id);
                         ...
                         .Filter((uofw, q, id, oid) => q.Where(w => w.CadastralID == id && w.EstateTyepLocation == EstateTyepLocation.Construction)).Visible(true).Order(3))
              ...
              .AddOneToManyAssociation<Right>("RealEstateComplex_Rights",
               ...
               //.AddOneToManyAssociation<FileCard>("RealEstateComplex_RightDocs",
               ...
               .AddOneToManyAssociation<Encumbrance>("CadastralObject_Encumbrances",
               ...
               .AddOneToManyAssociation<AccountingObject>("RealEstateComplex_AccountingObjects",
                        editor => editor
               ...
               .Config.DetailView.Editors
                  .AddManyToMany("RealEstateComplex_FileCards"
               ...
               //выписки                 
               .AddManyToMany("RealEstateComplex_Extracts"
        ...
        /// <summary>
        /// Конфигурация реестра кадастрового объекта по умолчанию.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<RealEstateComplex> ListView_Default(this ViewModelConfigBuilder<RealEstateComplex> conf)
        {
            return
                conf.ListView(x => x
                .Title("Кадастровые объекты")
                ...
                .Columns(col => col
                    ...
                    //.Add(c => c.DateOfReceipt, ac => ac.Title("Дата оприходования").Visible(true).Order(15))
                    //.Add(c => c.EstateType, ac => ac.Title("Класс КС").Visible(true).Order(16))
                    //.Add(c => c.OKTMORegion, ac => ac.Title("Регион по ОКТМО").Visible(true).Order(17))
                    .Add(c => c.RighHolder, ac => ac.Title("Правообладатель").Visible(true).Order(130))
                    .Add(c => c.SibRegion, ac => ac.Title("Регион").Visible(true).Order(140))
                    .Add(c => c.RighKindAndShare, ac => ac.Title("Вид права, доля в праве").Visible(true).Order(150))
                    .Add(c => c.RightRegDate, ac => ac.Title("Дата гос.регистрации").Visible(true).Order(160))
                    .Add(c => c.RightRegNumber, ac => ac.Title("Номер гос.регистрации").Visible(true).Order(170))
                    ...
                    .Add(c => c.DealProps, ac => ac.Title("Реквизиты договора").Visible(true).Order(180))
                    ...
                    .ColumnsFrom<AdditionalFeatures>(RealEstateComplex_AdditionalFeatures, nameof(IHasAdditionalFeature.AdditionalFeaturesID), nameof(AdditionalFeatures.ID))
                );

        }

        /// <summary>
        /// Конфигурация реестра кадастрового объекта для мнемоники реестра, отображаемого при открытии пункта навигационного меню.
        /// </summary>
        /// <param name="conf"></param>
        /// <returns></returns>
        public static ViewModelConfigBuilder<RealEstateComplex> ListView_RealEstateComplexMenuList(this ViewModelConfigBuilder<RealEstateComplex> conf)
        {
            return
