...
namespace CorpProp.Entities.Subject
{
    ...
    public class Society : TypeObject, ISuperObject<Society>
    {
        private static readonly CompiledExpression<Society, string> _Name =
          DefaultTranslationOf<Society>.Property(x => x.Name).Is(x => x.IDEUP + " " + x.ShortName);


        private static readonly CompiledExpression<Society, string> _contactResponsableForResponse =
                DefaultTranslationOf<Society>.Property(x => x.ContactResponsableForResponse).Is(x => x.ResponsableForResponse == null ? null : x.ResponsableForResponse.Phone);
        private static readonly CompiledExpression<Society, string> _subdivisionResponsableForResponse =
                DefaultTranslationOf<Society>.Property(x => x.SubdivisionResponsableForResponse).Is(x => x.ResponsableForResponse == null ? null : x.ResponsableForResponse.SocietyDeptName);

        private static readonly CompiledExpression<Society, bool?> _isCauk = DefaultTranslationOf<Society>.Property(x => x.IsCAUK).Is(x => x.IDEUP == "1");
        private static readonly CompiledExpression<Society, bool?> _isService = DefaultTranslationOf<Society>.Property(x => x.IsService).Is(x => x.IDEUP == "90");


        #region Contstructor
        /// <summary>
        /// Инициализирует новый экземпляр класса Society.
        /// </summary>
        public Society() : base()
        {
        }
