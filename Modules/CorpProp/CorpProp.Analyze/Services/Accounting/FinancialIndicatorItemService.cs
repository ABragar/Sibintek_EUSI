...
{
    public interface IFinancialIndicatorItemService : ITypeObjectService<FinancialIndicatorItem>, IExcelImportEntity, ISystemImportEntity
    {
    }

    public class FinancialIndicatorItemService : TypeObjectService<FinancialIndicatorItem>, IFinancialIndicatorItemService
    {
        private readonly ILogService _logger;

        public FinancialIndicatorItemService(IBaseObjectServiceFacade facade, ILogService logger) : base(facade, logger)
        {
            _logger = logger;
        }

        protected override IObjectSaver<FinancialIndicatorItem> GetForSave(IUnitOfWork unitOfWork,
            IObjectSaver<FinancialIndicatorItem> objectSaver)
        {
            return base.GetForSave(unitOfWork, objectSaver)
                .SaveOneObject(account => account.Owner)
                .SaveOneObject(account => account.FinancialIndicator);
        }
            ...
            , ref ImportHistory history)
        {
            ...
            {
                bool isNew = true;
                FinancialIndicatorItem obj = null;

                obj = Activator.CreateInstance<FinancialIndicatorItem>();
                obj.FillObject(uofw, typeof(FinancialIndicatorItem), row, row.Table.Columns, ref error,
        ...
        public FinancialIndicatorItem CreateFromImport(
            IUnitOfWork uow
            , FinancialIndicatorItem obj
