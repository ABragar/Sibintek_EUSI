...
var ListView = WrapWidget.extend({
    ...
    init: function (options) {
        ...
        this.transport.read.url = URI.expand('/api/listview/{mnemonic}/{type}/{categorized}',
            {
                type: options.type,
        ...
        this.transport.createDefault.url = URI.expand('/api/standard/{mnemonic}/create_default',
            {
                mnemonic: this.mnemonic
            });
    ...
    setTransport: function (transport) {
        ...
        if (transport.read) {
            if (transport.read.url)
                this.transport.read.url = transport.read.url;
        ...
        if (transport.createDefault) {
            if (transport.createDefault.url)
                this.transport.createDefault.url = transport.createDefault.url;
    ...
    addEntity: function (params) {
        ...
        var createDefault = $.ajax({
            url: this.transport.createDefault.url + '?' + $.param(this.transport.createDefault.data),
            method: 'GET',
    ...
    dataSourceRead: function () {
        ...
        var url = this.transport.read.url;
        ...
        if (!this.widget().dataSource.transport.options.read.url || ((url.search('oneToManyAssociation') !== -1 && typeof url.search('oneToManyAssociation') !== "object") || (url.search('manyToManyAssociation') !== -1 && typeof url.search('manyToManyAssociation') !== "object")))
            this.widget().dataSource.transport.options.read.url = url;

        if (!this.widget().dataSource.options.transport.read.url || ((url.search('oneToManyAssociation') !== -1 && typeof url.search('oneToManyAssociation') !== "object") || (url.search('manyToManyAssociation') !== -1 && typeof url.search('manyToManyAssociation') !== "object")))
            this.widget().dataSource.options.transport.read.url = url;

        
        this.widget().dataSource.transport.options.read.cache = false;

        this.widget().dataSource.options.transport.read.data = function () {
            return readData;
        };
        ...
        for (var i = 0; i < nonFooterRows.length; i++) {
            ...
            for (var y = 0; y < row.cells.length; y++) {
                ...
                if (cell.value && typeof cell.value === "object" && !(cell.value instanceof Date)) {
                    cell.value = cell.value.Name || cell.value.ShortName;
                }
            }
        };
