...
namespace Base.BusinessProcesses.Services.Concrete
{
    public class ProductionCalendarService : IProductionCalendarService
    {
        ...
        public DateTime GetEndDate(DateTime start, TimeSpan period, PerfomancePeriodType periodType)
        {
            if (periodType == PerfomancePeriodType.Calendar) return start.AddMinutes(period.TotalMinutes);
            ...
            using (var uofw = _unitOfWorkFactory.CreateSystem())
            {
                ...
                while (period.TotalDays > 0)
                {
                    dtmEnd = dtmEnd.AddDays(1);

                    if (weekends.All(x => x.Date != dtmEnd.Date))
                    {
                        period = period.Subtract(TimeSpan.FromDays(1));
                    }
                }

                return dtmEnd.AddMinutes(period.TotalMinutes);
            }
        }
