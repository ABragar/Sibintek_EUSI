...
namespace Base.Task.Services
{
    public class TaskReportService : BaseCategorizedItemService<TaskReport>, ITaskReportService
    {
        ...
        private IQueryable<Entities.Task> GetAllTasks(IUnitOfWork unitOfWork, int categoryId = 0)
        {
            ...
            {
                var strId = HCategory.IdToString(AppContext.SecurityUser.CategoryInfo.ID);

                tasks = tasks.Where(x => x.AssignedTo.Category_.sys_all_parents.Contains(strId) || x.AssignedTo.CategoryID == AppContext.SecurityUser.CategoryInfo.ID);
            }
