...
#pragma warning restore 0219
        ...
        {
            Expression left = ParseInBetween();
        ...
        // between operator tested for DateTime and DateTime? only
        ...
        Expression ParseInBetween()
        {
            ...
            while (TokenIdentifierIs("between") || TokenIdentifierIs("not_between"))
            {
                var op = _token;
                ...
                if (_token.id == TokenId.OpenParen)
                {
                    ...
                    while (_token.id != TokenId.CloseParen)
                    {
                        ...
                        if (accumulate.Type != typeof(bool))
                        {
                            if (TokenIdentifierIs(op, "between"))
                            {
                                accumulate = GenerateGreaterThanEqual(identifier, right);
                            }
                            else if (TokenIdentifierIs(op, "not_between"))
                            {
                                accumulate = GenerateLessThanEqual(identifier, right);
                            }
                        }
                        else
                        {
                            if (TokenIdentifierIs(op, "between"))
                            {
                                accumulate = Expression.AndAlso(accumulate, GenerateLessThanEqual(identifier, right));
                            }
                            else if (TokenIdentifierIs(op, "not_between"))
                            {
                                accumulate = Expression.Or(accumulate, GenerateGreaterThanEqual(identifier, right));
                            }
                        }
        ...
        Expression ParseRealLiteral()
        {
            ...
            else
            {
                ...
                if (Double.TryParse(text, NumberStyles.Float, info,out d)) value = d;
            }
        ...
        Expression ParseIdentifier()
        {
                ...
                _externals != null && _externals.TryGetValue(_token.text, out value))
            {
                if (value != null && value.GetType().IsPrimitive)
                {
                    ...
                    var property = Expression.Property(Expression.Constant(obj), "Value");
                    var castExpr = Expression.Convert(property, value.GetType());
        ...
        {
            return TokenIdentifierIs(_token, id);
        }
