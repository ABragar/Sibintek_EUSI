...
namespace System.Linq.Dynamic
{
    ...
    public static class DynamicQueryable
    {
        ...
        /// </summary>
        /// <param name="source">A sequence of values to project.</param>
        /// <param name="collectionSelector">A projection string expression to apply to each element.</param>
        /// <param name="resultSelector">A projection function to apply to each element of each intermediate sequence.</param>
        /// <param name="args">An object array that contains zero or more objects to insert into the predicate as parameters.  Similiar to the way String.Format formats strings.</param>
        /// <returns>An <see cref="IQueryable"/> whose elements are the result of invoking a one-to-many projection function on each element of the input sequence.</returns>
        public static IQueryable SelectMany(this IQueryable source, string collectionSelector, string resultSelector, params object[] args)
        {
            Validate.Argument(source, "source").IsNotNull().Check()
            ...
            LambdaExpression collectionLambda = DynamicExpression.ParseLambda(source.ElementType, null, collectionSelector, args);

            //we have to adjust to lambda to return an IEnumerable<T> instead of whatever the actual property is.
            Type inputType = source.Expression.Type.GetGenericArguments()[0];
            Type resultType = collectionLambda.Body.Type.GetGenericArguments()[0];
            Type enumerableType = typeof(IEnumerable<>).MakeGenericType(resultType);
            Type delegateType = typeof(Func<,>).MakeGenericType(inputType, enumerableType);

            collectionLambda = Expression.Lambda(delegateType, collectionLambda.Body, collectionLambda.Parameters);

            //we have to create additional lambda for result selection
            var xName = "outer";
            var yName = "inner";
            ParameterExpression xParameter = Expression.Parameter(source.ElementType, xName);
            ParameterExpression yParameter = Expression.Parameter(resultType, yName);

            LambdaExpression resultSelectLambda = DynamicExpression.ParseLambda(new[] { xParameter, yParameter }, null, resultSelector, args);
            Type resultLambdaResultType = resultSelectLambda.Body.Type;

            return source.Provider.CreateQuery(
                Expression.Call(
                typeof(Queryable), nameof(Queryable.SelectMany),
                new[] { source.ElementType, resultType, resultLambdaResultType
    },
                source.Expression, Expression.Quote(collectionLambda), Expression.Quote(resultSelectLambda))
                );
        }
        #endregion
        ...
        /// <param name="innerKeySelector">A dynamic function to extract the join key from each element of the second sequence.</param>
        /// <param name="resultSelector">A dynamic function to create a result element from two matching elements.</param>
        public static IQueryable LeftOuterJoin(this IQueryable outer, IQueryable inner, string outerKeySelector, string innerKeySelector, string resultSelector)
        {
            Validate.Argument(outer, nameof(outer)).IsNotNull().Check()
                .Argument(inner, nameof(inner)).IsNotNull().Check()
                .Argument(outerKeySelector, nameof(outerKeySelector)).IsNotNull().IsNotEmpty().IsNotWhiteSpace().Check()
                .Argument(innerKeySelector, nameof(innerKeySelector)).IsNotNull().IsNotEmpty().IsNotWhiteSpace().Check()
                .Argument(resultSelector, nameof(resultSelector)).IsNotNull().IsNotEmpty().IsNotWhiteSpace().Check();

            return outer.GroupJoin(inner, outerKeySelector, innerKeySelector, $"new (outer as outer, group as Group)")
        ...
        /// <param name="values">An object array that contains zero or more objects to insert into the predicates as parameters. Similar to the way String.Format formats strings.</param>
...
null, resultSelector, values);
            ...
            return outer.Provider.CreateQuery(Expression.Call(typeof(Queryable),
                "GroupJoin", new[] { outer.ElementType, innerElementType, outerLambda.Body.Type, resultLambda.Body.Type
},
                outer.Expression,
        ...
        public static IQueryable Cast(this IQueryable query)
        {
            var method = typeof(Queryable).GetMethod("Cast");

            var generic = method?.MakeGenericMethod(query.ElementType.GetCorrectBaseType());

            return generic?.Invoke(null, new object[] { query }) as IQueryable;
        }

        public static Type GetCorrectBaseType(this Type type)
        {
