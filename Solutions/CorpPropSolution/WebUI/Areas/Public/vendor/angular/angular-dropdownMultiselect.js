    ...
    .directive( 'dropdownMultiselect', [ function () {
        return {
            ...
            template: '<div class="dropdown" id="dropdownMultiselect">' +
                ...
                '<span class="pull-left"  ng-if="!hasText">Выбрать </span>' +
                ...
                '<li><a ng-click="selectAll()"><i class="glyphicon glyphicon-ok"></i> Выбрать все</a></li>' +
                '<li><a ng-click="unSelectAll()"><i class="glyphicon glyphicon-remove"></i> Отменить выбор</a></li>' +
                ...
                '<a ng-click="setSelectedItem($event)">' +
                '<span ng-class="isChecked(option[trackByKey], dropdownType)"></span> {{option[leftKey]}} {{divider}} {{option[rightKey]}}' +
                '</a>' +
                '</li>' +
                '</ul>' +
                '</li>' +
            ...
            controller: function ($scope) {
                //debugger;
                var model, badgeVisibility = true, // default badge visibility
                ...
                $scope.model = model = angular.isDefined($scope.model) ? $scope.model : [];
                ...
                try {
                    ...
                    var unbindWatcher = $scope.$watch( 'options', function ( newVal, oldVal ) {
                        ...
                        if (newVal && angular.isDefined(oldVal) && !angular.equals(newVal, oldVal)) {
                            model = [];
                            $scope.model = model;
                        }
                ...
                document.addEventListener( 'click', function ( event ) {
                    ...
                    if ( parentTarget !== null ) {
                        ...
                        while (eventSrc !== dropdownMultiselect) {
                            //console.log('Something wrong with this while...');
                            if (angular.isUndefined(eventSrc)) break;

                            eventSrc = eventSrc['offsetParent'];

                            if (eventSrc['offsetParent'] === null) {
