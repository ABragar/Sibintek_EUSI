...
namespace WebUI.Controllers
{
    ...
    public class ResponseController : BaseController
    {
        ...
        public JsonNetResult GetGridResponseData([DataSourceRequest] DataSourceRequest request, int? requestId = null, int? responseId = null, bool? societyExport = false)
        {
            if (requestId == null && responseId == null)
            ...
            using (var uofw = CreateUnitOfWork())
            {
                ...
                if (requestId != null)
                {
                    ...
                    data = requestSelector.AppendSociety(data, societyExport ?? false ? nameof(Society.ShortName) : nameof(Society.ID), SocietyIdFieldName);
                    if (data == null)
                        return new JsonNetResult(new object[0]);
                }
