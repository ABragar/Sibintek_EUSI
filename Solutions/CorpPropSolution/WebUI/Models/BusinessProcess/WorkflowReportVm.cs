...
namespace WebUI.Models.BusinessProcess
{
    public class WorkflowReportVm
    {
        ...
        public WorkflowReportVm(Workflow wfl, IEnumerable<ChangeHistory> histories)
        {
            ...
            this.CountOfObjectsByStage = histories.GroupBy(x => x.WorkflowVersionID).Select(x => x.OrderByDescending(h => h.SortOrder).FirstOrDefault()).GroupBy(x => x.Step).Select(x => new CountOfObjectsReportVm
            {
