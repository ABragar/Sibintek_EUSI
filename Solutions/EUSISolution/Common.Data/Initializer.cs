...
namespace Common.Data
{
    public class CommonDataInitializer : IModuleInitializer
    {
        ...
        public void Init(IInitializerContext context)
        {
            ...
            //context.ModifyVmConfig<User>("AccessUserForm").DetailView(dv => dv.Wizard("СoncreteAccessWizard"));            
            context.ModifyVmConfig<User>("AccessUserForm")
               ...
               .Editors(ed =>
               {
                   ...
                   ed.Add(pr => pr.IsActive, ac => ac.Visible(false).Title("Активный"));
              ...
              .Columns(col => col.Add(c => c.SysName, ac => ac.Visible(true).Order(0).Title("Логин"))
                               ...
                               .Add(c => c.IsActive, ac => ac.Visible(false).Order(2).Title("Активный"))
                               .Add(c => c.Image, ac => ac.Visible(false))
              ))
              .LookupProperty(x => x.Text(e => e.SysName));

            context.ModifyVmConfig<User>("AccessUserAD")
                .DetailView(dv => dv.Wizard(nameof(ADUserWizard))
               .Editors(ed =>
               {
                   ed.Add(pr => pr.Image, ac => ac.Visible(false));
                   ed.Add(pr => pr.FullName, ac => ac.Visible(false));
            ...
            context.DataInitializer("Data", "0.1", () =>
            {
                ...
                UsersAndRolesInitializer.Seed(context.UnitOfWork, _loginProvider, _presetRegistorService, presets);
            });
        }
    }
}