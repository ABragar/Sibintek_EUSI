    ...
    public static class KendoTreeListViewHelper
    {
        ...
        public static TreeListColumnFactory<TModel> InitEnumColumn<TModel>(this TreeListColumnFactory<TModel> factory, ColumnPreset columnPreset, ColumnViewModel column, StandartGridView gridView) where TModel : class
        {
            ...
            factory.Add()
                ...
                .Template(
                    string.Format(
                        "<i class='#=!application.UiEnums.getValue('{0}', data.{1}) ? '' : application.UiEnums.getValue('{0}', data.{1}).Icon#' style='width: 20px; color: #=!application.UiEnums.getValue('{0}', data.{1}) ? '' : application.UiEnums.getValue('{0}', data.{1}).Color#;'></i><span>#=!application.UiEnums.getValue('{0}', data.{1}) ? '' : application.UiEnums.getValue('{0}', data.{1}).Title#<span>",
        ...
        public static DataSourceTreeListModelDescriptorFactory<T> InitModel<T>(this DataSourceTreeListModelDescriptorFactory<T> datasource, WebViewPage<StandartTreeListView> webViewPage) where T : class
        {
            ...
            var presetPropNames = preset.Columns.Where(c => c.Visible).Select(c => c.Name);

            var defaultColumns = webViewPage.Model.ViewModelConfig.ListView.Columns.Where(c => presetPropNames.Contains(c.PropertyName));

            foreach (var column in defaultColumns)
            {
                datasource.Field(column.PropertyName, column.PropertyType);
            }
