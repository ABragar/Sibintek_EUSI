        ...
        public static void Init(IColumnBoundRegisterService boundRegisterService)
        {
            boundRegisterService
                ...
                .Create((builder, preset, column, grid) =>
                {
                    ...
                    string clTemplate = (!String.IsNullOrEmpty(column.ClientTemplate)) ? column.ClientTemplate :
                    $"#= pbaAPI.truncate(pbaAPI.htmlEncode(data.{column.PropertyName}), 300) #";
                    builder
                        .ClientTemplate(clTemplate)
                        .Filterable(f =>
                        {
            ...
            boundRegisterService
               .Register(PropertyDataType.ObjectTypeName)
               .InitDefault()
               .CustomBoundType(typeof(string))
               .Create((builder, preset, column, grid) =>
               {
            ...
            boundRegisterService
                ...
                {
                    builder
                        .ClientTemplate($"#=gridTemplates.period({column.PropertyName}.Start, {column.PropertyName}.End, '{grid.Mnemonic}')#")
            ...
            boundRegisterService
                ...
                .Create((builder, preset, column, grid) =>
                {
                    string clTemplate = (!String.IsNullOrEmpty(column.ClientTemplate)) ? string.Format(column.ClientTemplate, column.PropertyName) :
                    string.Format("#= data.{0} != null ? kendo.toString(data.{0}, 'n') : '' #", column.PropertyName);
                    builder
                        .ClientTemplate(clTemplate)
            ...
            boundRegisterService
               ...
               {
                   builder
                       .ClientTemplate(string.Format("#= data.{0} != null ? kendo.toString(kendo.parseDate(data.{0}, '{1}'), '{2}') : '' #", column.PropertyName, "HH:mm:ss", JsonNetResult.TIME_FORMATE))
                       .Filterable(f =>
                       {
                           f.Enabled(false);                          
