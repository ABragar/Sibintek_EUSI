...
$.fn.pbaForm = function (options) {
    ...
    //events
    form.onShown = function (id) {
        if (!form.__active) {
            form.__events['onShown'] = {
                func: form.onShown
            };
    ...
    form.onTabShown = function (tabID) {
        //TODO: исправить - не прогружаются ассоциации
        // если форма не активна, то выход и не отрабатывает onTabShown для ассоциаций.
        //if (!form.__active) {
        //    form.__events['onTabShown'] = {
        //        func: form.onTabShown,
        //        args: tabID
        //    };
    ...
    form.onActivate = function () {
        form.__active = true;

        for (var key in form.__events) {
            var event = form.__events[key];
            event.func.call(form, event.args);
        }
