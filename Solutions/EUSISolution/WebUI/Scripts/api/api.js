...
(function () {
    ...
    $.extend(pbaAPI,
    {
        ...
        initViewModel: function (config, params) {
            ...
            var kendWindow = $w.data('kendoWindow');
        ...
        selectSimple: function (items, params) {
            ...
            if (!$w.length) {
                ...
                if (_params.multiSelect) {
                    ...
                    var html =
                        "<div id='" + contentId + "'>" +
                            "<div class='dialog'> " +
                                ...
                                "</div>" +
                        "<ul id=\"" + _params.wid + "\" class=\"view-model-window kwidget kwidget--list scrollable\"></ul>" +
                            "</div>" +
                        "</div>";
                    body.append(html);
                    ...
                    body.append("<ul id=\"" + _params.wid + "\" class=\"view-model-window kwidget kwidget--list scrollable\"></ul>");
                    $w = $("#" + _params.wid);
                }
            }
        ...
        openDetailView: function (mnemonic, params) {            
            ...
            if (mnemonic == 'Subject' && params.id) {
                ...
                application.viewModelConfigs.get(mnemonic).done(
                    function (config) {
                        if ((config.Ext.Relations && !params.isCorrectMnemonic) || !params.id)
                            pbaAPI.openDv(mnemonic, params, config);
                        else
                            pbaAPI.proxyclient.EUSI.checkEntityBlock({ entityId: params.id, entityMnemonic: config.Mnemonic }).done(function (res) {
                                var unblockFunction = function () {
                                    pbaAPI.proxyclient.EUSI.removeEntityBlock({ entityId: params.id, entityMnemonic: config.Mnemonic });
                                };

                                //Event на закрытие окна.
                                window.removeEventListener("beforeunload", unblockFunction);
                                window.addEventListener("beforeunload", unblockFunction);

                                if (res.error)
                                    return pbaAPI.errorMsg(res.error);

                                //Скрытие кнопок сохранения.
                                params.isBlocked = res.isBlocked;

                                var $w = pbaAPI.openDv(mnemonic, params, config);

                                if (res.isBlocked)
                                    pbaAPI.alertWarning("Внимание! В данный момент с запрашиваемым объектом уже работает пользователь " + res.userFullName + ".", function () {
                                        pbaAPI.confirm("Продолжить?", "Внесенные изменения не будут сохранены. Продолжить работать с объектом?", null, function (z, x, c) {
                                            $form.parent().closest(".view-model-window").data("kendoWindow").close();
                                        });
                                    });

                                //Event закрытия модального окна.
                                if ($w) 
                                    $w.parent().find(".k-window-titlebar").find(".k-icon.k-i-close").click(unblockFunction);
                            });
        ...
        openDv: function (mnemonic, params, config) {
            var _params = $.extend({
                ...
                byDate: null,
                parentLV: null,
                withoutRelations: null,
                isBlocked: null
            },
                params);

            if (_params.withoutRelations) {
                delete config.Ext.Relations;
            }
            ...
            if (isNew && wizardName) {
                ...
                wnd.bind("close",
                    function (e) {
                        ...
                        var changeObjects = dialog.changeObjects;
                ...
                if ($.isArray(_params.ids)) {
                    for (i = 0; i < _params.ids.length; i++) {
                        var idarr = _params.ids[i];

                        if (idarr && idarr !== 0) {
                            entities[idarr] = { model: null, order: i };
                        }
                    }
        ...
        openWizardViewModelEx: function (mnemonic, params) {
            ...
            application.viewModelConfigs.get(mnemonic).done(
                function (config) {
                    ...
                    if ($.isArray(_params.ids)) {
                        for (var a = 0; a < _params.ids.length; a++) {
                            var id = _params.ids[a];

                            if (id && id !== 0) {
                                entities[id] = { model: null, order: a };
                            }
                        }
                    } else if (_params.entities) {
                        for (var i = 0; i < _params.entities.length; i++) {
