...
namespace WebUI
{
    public class Startup
    {
        public void Configuration(IAppBuilder app)
        {
            ...
            locator.GetService<Base.Service.Log.ILogService>().Log("Application Start");

#if (Release)
            app.UseWindowsAuth(locator);
#else
            if (ADSettingsHelper.IsWindowsAuthentification())
            {
                app.UseWindowsAuth(locator);
            }
            else
