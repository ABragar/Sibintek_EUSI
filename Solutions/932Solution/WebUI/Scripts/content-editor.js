...
(function () {
    ...
    contentEditor.ContentWidget = kendo.Class.extend({
    ...
    kendo.data.binders.content_img = kendo.data.Binder.extend({
        ...
        refresh: function () {
            ...
            if (!url) {
                ...
                if (file && file.FileID) {
                    url = pbaAPI.imageHelpers.getsrc(file.FileID, width, height) + '&type=frame';
                    //TODO: добавил эту хероту, чтобы сохранялось добавляемое изображение в контент-эдиторе
    ...
    kendo.data.binders.content_file = kendo.data.Binder.extend({
        refresh: function () {
            ...
            if (file && file.FileID !== "00000000-0000-0000-0000-000000000000") {
                $anchor
                    .attr('href', pbaAPI.getHrefFile(file.FileID))
                    .attr('title', file.FileName)
                    .attr('file-type', file.Extension)
                    //.attr('file-size', file.Size)
                    .html(file.parent().Title || file.FileName);
            } else {
                $anchor
                    .attr('href', "#")
