// Last time updated at May 09, 2015, 08:32:23
...
(function () {
    ...
    window.RTCMultiConnection = function (channel) {
        ...
        connection.leave = function (userid, callback) {
            ...
            rtcMultiSession.leave(callback);
        };
    ...
    function RTCMultiSession(connection, callbackForSignalingReady) {
        ...
        function newPrivateSocket(_config) {
            var socketConfig = {
                ...
                //chosenSession: _config.chosenSession,
                onopen: function (_socket) {
                    ...
                    if (!socket.__push) {
                        ...
                        socket.send = function (message) {
                            ...
                            message.chosenSession = message.chosenSession || connection.chosenSession;
            ...
            function updateSocket() {
                ...
                if (connection.customPeerAdd)
                    connection.customPeerAdd(_config);
            }
            ...
            function socketResponse(response) {
                ...
                if (response.sdp) {
                    ...
                    _config.chosenSession = response.chosenSession;
                ...
                if (response.streamid) {
                    //console.log('On stream ID:', response);
            ...
            for (var i = 0; i < labels.length; i++) {
                ...
                if (connection.streams[label]) {
                    try {
                        ...
                        console.error('Error remove peer:', e);
                    }
                }
            }
        }
        ...
        function onNewParticipant(response) {
            ...
            newPrivateSocket({
                ...
                //chosenSession: response.userData ? response.userData.chosenSession : response.chosenSession,
            });
        ...
        connection.remove = function (userid) {
            ...
            //connection.streams = Array.prototype.filter.call(connection.streams, function (stream) {
            //    if (stream.userid == userid) {
            //        onStreamEndedHandler(stream, connection);
            //        return false;
            //    }
            ...
            for (var stream in connection.streams) {
                ...
                if (connection.streams[stream].userid === userid) {
            ...
            for (var user in joinUsers) {
                if (!participants[joinUsers[user]]) {
                    onNewParticipant({
                        ...
                        userid: connection.userid,
                        extra: connection.extra,
                        //chosenSession: connection.chosenSession
                    });
                }
            }
        }
                    ...
                    if (response.customMessage) {
                        if (response.message.drop) {
                            ...
                            if (response.message.renegotiate) {
                                console.log('connection.renegotiate ...');
        ...
        function joinSession(_config, skipOnStateChange) {
            ...
            newPrivateSocket({
                ...
                extra: _config.extra || {},
                userid: _config.userid,
                choosenSession: connection.choosenSession
            });
        ...
        this.disconnect = function () {
            ...
            if (!connection.keepStreamsOpened) {
                for (var streamid in connection.localStreams) {
                    console.log('Local streams:', connection);
    ...
    function PeerConnection() {
        return {
            ...
            getLocalDescription: function (createType) {
                ...
                function createDescription() {
                    self.connection[createType == 'offer' ? 'createOffer' : 'createAnswer'](function (sessionDescription) {
                        ...
                        // !!!!!!!!!!!!!!!!!!!!!!
                        //sessionDescription.sdp = globalapp.rtcService.fixSdp(sessionDescription.sdp);
                        // !!!!!!!!!!!!!!!!!!!!!!
            ...
            addStream: function (stream) {
                ...
                console.log('Stream before sending:', stream);
                //stream.isAudio = true;
                //stream.isVideo = false;

                //switch (stream.customType) {
                //    case 'audio':
    ...
    function createMediaElement(stream, session) {
        console.log('Try to create element...');
                ...
                if (isFirefox) {
                    ...
                    if (session.video) {
                        self.videoRecorder = RecordRTC(self.stream, {
                            type: 'video',
                            video: {
                                width: session.width ? session.width : 320,
                                height: session.height ? session.height : 240
                            },
                            canvas: {
                                width: session.width ? session.width : 320,
                                height: session.height ? session.height : 240
